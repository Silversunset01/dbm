...
So, the only thing you'll need is a `Set Bot Game` or a `Set Bot Activity` (with beta) action. Create one of them, and set the text to: `${this.getDBM().Bot.bot.guilds.array().length} servers`. And.. that's it! Now run the command or put the action inside a On Bot Initialization event!  
![](https://i.imgur.com/zjSkZ56.png)

## How to use `client` in non-beta

Since `client` isn't defined in non-beta, you must use `this.getDBM().Bot.bot` to call the Discord.js Client. However, `this.getDBM().Bot.bot` is too long to type and we don't want do do that, do we? To workaround this, you can put the following code in a Run Script inside a Bot Initilization event.
```JavaScript
